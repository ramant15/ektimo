...
defaults.titleFormat.agendaList='W'; // or M
		 ...
		 // sort our display list, makes easier to display
            ...
            for (i in displayeventlist) {
                ...
                if ( em == vm ) {
                    ...
                    if(opt('showLocation')) {
                        lloc   = displayeventlist[i].location;
                    }
                    if(opt('showPhone')) {
                        lphone   = displayeventlist[i].phone;
                    }
                    if(opt('showUrl')) {
                        lmyurl   = displayeventlist[i].myurl;
                    }
                    
                    lcolor   = displayeventlist[i].color;
                    ...
                    if (allDay) {
                        eventdisplay = $("<li class='fc-agendaList-item fc-today fc-thu'>"+
                                            ...
                                            "<span style='background-color:"+lcolor+";padding-bottom:4px;'>&nbsp;</span>"+
                                            "<div class='fc-event-time'>"+
                                                "<span class='fc-event-all-day'></span>"+
                                            "</div>"+
                                            "<div class='fc-agendaList-eventDetails'>"+
                                              "<div class='fc-eventlist-title'>"+ltitle+"</div>"+
                                              (opt('showDescription') && ldesc !== '' ? "<div class='fc-eventlist-desc'>"+Lang.ListView.descriptionLabel+ ldesc+"</div>" : "")+
                                              (opt('showLocation') && lloc !== '' ? "<div class='fc-eventlist-desc'>"+Lang.ListView.locationLabel+lloc+"</div>" : "")+
                                              (opt('showPhone') && lphone !== '' && lphone !== null ? "<div class='fc-eventlist-desc'>"+Lang.ListView.phoneLabel+lphone+"</div>" : "")+
                                              (opt('showUrl') && lmyurl !== '' && lmyurl !== null ? "<div class='fc-eventlist-desc'>"+Lang.ListView.urlLabel+": <a href='"+lmyurl+"' target='_blank'>"+lmyurl+"</a></div>" : "")+
                                               "</div>"+
                                          "</" + (lurl ? "a" : "div") + ">"+
                                        "</li>").appendTo(html);
                    } else {
...
function AgendaEventRenderer() {
    ...
    function slotEventEnd(event) {
		if (event.end) {
            ...
            if(event.time_end == '00:00:00') {
                // Paul: fixes bug with endtime 00:00:00, midnight
                var st_clone = cloneDate(event.start);
                st_clone.setHours(23);
                st_clone.setMinutes(59);
                st_clone.setSeconds(59);
                return st_clone;
            } else {
	...
	function slotSegHtml(event, seg) {
		...
		html +=
			...
			"</div>" +
            "<div class='fc-event-description'></div>" +
			"</div>" +
...
function DayEventRenderer() {
	...
	function buildHTMLForSegment(segment) {
		var cuurentOrder = localStorage.getItem('order_id');

		var html = '';
		var isRTL = opt('isRTL');
		var event = segment.event;
		var url = event.url;
		if(cuurentOrder ==  event.order_id ){
			var orderClass = 'current';
		}else{
			var orderClass = 'other';
		}
		...
		html +=
			...
			" style=" +
				...
				"'" +
			">" +
			"<div class='fc-event-inner "+orderClass+"'>";
		if(opt('showTitleFirst')) {	// Paul Wolbers
			html +=
	           "<span class='fc-event-title'>" +
				  htmlEscape(event.title || '') +
				"</span>" +
				"<span class='fc-event-tech'> (" +
				  htmlEscape(event.cal_name || '')+
				")</span>";
            ...
            html += "<span class='fc-event-description'></span>";
	        html +=    "</div>";
		} else {
			if (!event.allDay && segment.isStart) {
				html +=
			...
			html +=
				...
				"<span class='fc-event-tech'> (" +
				  htmlEscape(event.cal_name || '')+
				"</span>)" +
                "<span class='fc-event-description'></span>" +
				"</div>";
		}
...
function compareDaySegments(a, b) {
	if (a.event.sorter != null && b.event.sorter != null) {
		var sorter = a.event.sorter;
		if(typeof sorter == 'string') {
			return (b.rightCol - b.leftCol) - (a.rightCol - a.leftCol) || // put wider events first
			(a.event.sorter || '').localeCompare(b.event.sorter) ||
			a.event.start - b.event.start || // if a tie, sort by event start date
			(a.event.title || '').localeCompare(b.event.title);	//now take the sorter
		} else {
			return (b.rightCol - b.leftCol) - (a.rightCol - a.leftCol) || // put wider events first
			a.event.sorter - b.event.sorter ||
			a.event.start - b.event.start || // if a tie, sort by event start date
			(a.event.title || '').localeCompare(b.event.title);	//now take the sorter
		}
