	...
	<div class="row">
		<div class="col-md-10 col-md-offset-1">
			<div class="panel panel-default">
				...
				@endif 
				<form class="form-horizontal" method="POST" action="{{URL::to('customers/siteUpdate')}}">
					...
					<div class="scheduler-border">
						...
						<div class="form-group required">
							...
							<div class="col-md-5">
								<input type="text" id="company_name" class="form-control" name="company_name" value="{{$site->company_name}}">
							</div>
						</div>
						 <div class="form-group required">
							<label class="col-md-4 control-label" for="company_name">Site Name</label>
							<div class="col-md-5">
								<input type="text" id="company_name" class="form-control" name="name" value="{{$site->name}}">
							</div>
						</div>
							<div class="form-group required">
								<label class="col-md-4 control-label" for="address">Site Address</label>
								<div class="col-md-5">
									<textarea id="address" class="form-control" name="address" rows="2">{{$site->address}}</textarea>
								</div>
							</div>
							<div class="form-group required">
							...
							<div class="col-md-5">
								<select class="form-control" id="test" name="state_id">
									...
									<option  value="{{$value->id}}" @if($site->state_id == $value->id) selected @endif > {{ $value->name}}</option>
									@endforeach
								</select>
							</div>
						</div>
							<div class="form-group required">
								...
								<div class="col-md-5">
									<input type="text" id="telephone_number" class="form-control" name="phone" value="{{$site->phone}}">
								</div>
							</div>
							<div class="form-group required">
								<label class="col-md-4 control-label" for="email_address">Site Email </label>
								<div class="col-md-5">
									<input type="email" id="email_address" class="form-control" name="email" value="{{$site->email}}">
								</div>
							</div>
							<div class="form-group required">
								<label class="col-md-4 control-label" for="email_address">Site Contact </label>
								<div class="col-md-5">
									<input type="text" id="contact" class="form-control" name="contact" value="{{$site->contact}}">
								</div>
							</div>
							<div class="form-group required">
								<label class="col-md-4 control-label" for="email_address">Description </label>
								<div class="col-md-5">
									<textarea id="address" class="form-control" name="description" rows="2">{{$site->description}}</textarea>
								</div>
							</div>
						</div> 
						...
						<div class="pull-right">
								<div class="col-md-12">
									...
									&nbsp;
									<a href="{{URL::to('order')}}?c=<?php echo Request::input('c'); ?>&st={{$site->id}}" class="btn btn-primary">Cancel</a>
								</div>
						</div>  
